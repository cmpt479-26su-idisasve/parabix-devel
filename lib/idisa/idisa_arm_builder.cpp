...
Value * IDISA_ARM_Builder::neon_vld1x4()
{
  ...
  //create a static vector of {0,1,2,3} that represents the shift ammounts
  static const std::vector<Value *> shuffle_ammount = {ConstantInt::get(getInt32Ty(), 0)
                              ,ConstantInt::get(getInt32Ty(), 1)
                              ,ConstantInt::get(getInt32Ty(), 2)
                              ,ConstantInt::get(getInt32Ty(), 3)
                              };
  //create a function call
  return CreateCall(shiftl_f32func->getFunctionType(), shiftl_f32func, shuffle_ammount);
}
...
{
  Function* shiftl_u_f32func = Intrinsic::getDeclaration(getModule(), Intrinsic::arm_neon_vrshiftn);
