...
unsigned IDISA_SVE_Builder::NativeBitBlockWidth() { return codegen::HostSVEBitWidth(); }
...
llvm::Value *IDISA_SVE_Builder::simd_popcount(unsigned fw, llvm::Value *a) {
    ...
    if ((getVectorBitWidth(a) == mNativeBitBlockWidth) && (fw >= 8) && (fw <= 64)) {
        unsigned minN = ARM_SVE_min_width / fw;
        Function *cntIntrinsic = Intrinsic::getOrInsertDeclaration(getModule(), Intrinsic::aarch64_sve_cnt);
...
llvm::Value *IDISA_SVE_Builder::mvmd_shuffle(unsigned fw, llvm::Value *data_table, llvm::Value *index_vector,
                                             ShuffleMode m) {
    // TODO JL implement
    return IDISA_ARM_Builder::mvmd_shuffle(fw, data_table, index_vector, m);
}
...
llvm::Value *IDISA_SVE_Builder::mvmd_shuffle2(unsigned fw, llvm::Value *table0, llvm::Value *table1,
    ...
    // TODO JL implement
    return IDISA_ARM_Builder::mvmd_shuffle2(fw, table0, table1, index_vector, m);
}
