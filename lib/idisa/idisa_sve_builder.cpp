...
llvm::Value *IDISA_SVE_Builder::simd_popcount(unsigned fw, llvm::Value *a) {
    ...
    if ((vectorWidth <= mNativeBitBlockWidth) && (fw >= 8) && (fw <= 64)) {
        ...
        Function *svePtrue = Intrinsic::getOrInsertDeclaration(
            getModule(), Intrinsic::aarch64_sve_ptrue, {svPTy});
        // Function *svePtrue = Intrinsic::getOrInsertDeclaration(
        //     getModule(), Intrinsic::aarch64_sve_whilelo, {vTy, svTy});

        Function *vectorExtract = Intrinsic::getOrInsertDeclaration(
            getModule(), Intrinsic::vector_extract, {vTy, svTy});
        Function *vectorInsert = Intrinsic::getOrInsertDeclaration(
            getModule(), Intrinsic::vector_insert, {svTy, vTy});
        Function *sveCnt = Intrinsic::getOrInsertDeclaration(
            getModule(), Intrinsic::aarch64_sve_cnt, {svTy});
        ...
        if (pattern == SV_ALL) {
            report_fatal_error(
                StringRef(
                    "simd_popcount: Vector size has no predicate pattern: ") +
                std::to_string(svN));
        }
        Constant *i32PredPat = ConstantInt::get(getIntNTy(32), pattern);

        Value *v1 = CreateCall(vectorInsert->getFunctionType(), vectorInsert,
                               {UndefValue::get(svTy), a, i64Zero});
        Value *pred =
            CreateCall(svePtrue->getFunctionType(), svePtrue, {i32PredPat});
        Value *v2 = CreateCall(sveCnt->getFunctionType(), sveCnt,
                               {UndefValue::get(svTy), pred, v1});
        Value *v3 = CreateCall(vectorExtract->getFunctionType(), vectorExtract,
                               {v2, i64Zero});
