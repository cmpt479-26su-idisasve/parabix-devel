...
void processing(const int &fd,
                       ...
                       unsigned int &numSamples)
    {
        ...
        std::vector<int16_t> first(numSamples);
        std::vector<int16_t> second(numSamples);
        ...
        {   
            ...
            first[i] = firstValue;
            second[i] = secondValue;
        }
        
        std::vector<int8_t> marker_1(numSamples);
        std::vector<int8_t> marker_2(numSamples);
        for (int i=1;i<first.size();++i)
        {
            marker_1[i] = abs(first[i-1] - first[i]) >= 5000;
            marker_2[i] = abs(second[i-1] - second[i]) >= 5000;
        }


        const int fd_out = open("out.wav", O_WRONLY | O_CREAT, 0666);
        write(fd_out, marker_1.data(), marker_1.size() *sizeof(int8_t));
        write(fd_out, marker_2.data(), marker_2.size() *sizeof(int8_t));
    }
