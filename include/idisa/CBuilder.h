...
constexpr inline bool is_power_2(const uint64_t n) { return ((n & (n - 1)) == 0) && n; }
...
class CBuilder : public llvm::IRBuilder<> {
  ...
  public:
    CBuilder(llvm::LLVMContext &C, const codegen::FeatureSet &featureSet);
    ...
    llvm::Module *getModule() const {
        ...
        if (bb) {
            ...
            assert("CBuilder has an insert point that is not contained within a llvm::Function" && f);
    ...
    // Equivalent to CreateUDiv(CreateAdd(number, CreateSub(divisor, llvm::ConstantInt::get(divisor->getType(), 1))),
    // divisor)
    ...
    llvm::Value *CreateUMin(llvm::Value *const a, llvm::Value *const b, const llvm::Twine Name = "") {
        if (LLVM_UNLIKELY(a == nullptr || a == b))
            return b;
    ...
    llvm::Value *CreateAlignedMalloc(llvm::Type *const type, llvm::Value *const ArraySize, const unsigned addressSpace,
                                     const unsigned alignment);

    llvm::Value *CreatePageAlignedMalloc(llvm::Type *const type, llvm::Value *const ArraySize = nullptr,
                                         const unsigned addressSpace = 0);

    llvm::Value *CreatePageAlignedMalloc(llvm::Value *const size);

    llvm::Value *CreateCacheAlignedMalloc(llvm::Type *const type, llvm::Value *const ArraySize = nullptr,
                                          const unsigned addressSpace = 0);
    ...
    llvm::CallInst *CreateMemZero(llvm::Value *const ptr, llvm::Value *const size,
                                  const unsigned alignment = unsigned{1}) {
    ...
    llvm::AllocaInst *CreateAllocaAtEntryPoint(llvm::Type *Ty, llvm::Value *ArraySize = nullptr,
                                               const llvm::Twine Name = "");

    llvm::AllocaInst *CreateAlignedAlloca(llvm::Type *const Ty, const unsigned alignment,
                                          llvm::Value *const ArraySize = nullptr);
    ...
    llvm::AllocaInst *CreateAlignedAllocaAtEntryPoint(llvm::Type *const Ty, const unsigned alignment,
                                                      llvm::Value *const ArraySize = nullptr);
    ...
    template <typename... Args> llvm::CallInst *CreatePrintfCall(const llvm::StringRef format, Args... args) {
    ...
    llvm::Value *CreateMMap(llvm::Value *const addr, llvm::Value *size, llvm::Value *const prot,
                            llvm::Value *const flags, llvm::Value *const fd, llvm::Value *const offset);
    ...
    enum Protect { NONE = 0, READ = 1, WRITE = 2, EXEC = 4 };
    ...
    enum class STD_FD { STD_IN = STDIN_FILENO, STD_OUT = STDOUT_FILENO, STD_ERR = STDERR_FILENO };

    void CallPrintIntCond(llvm::StringRef name, llvm::Value *const value, llvm::Value *const cond,
                          const STD_FD fd = STD_FD::STD_ERR);

    llvm::CallInst *CallPrintInt(llvm::StringRef name, llvm::Value *const value, const STD_FD fd = STD_FD::STD_ERR);

    llvm::CallInst *CallPrintRegister(llvm::StringRef regName, llvm::Value *const value,
                                      const STD_FD fd = STD_FD::STD_ERR);
    ...
    inline unsigned getCacheAlignment() const { return mCacheLineAlignment; }
    ...
    llvm::BranchInst *CreateLikelyCondBr(llvm::Value *Cond, llvm::BasicBlock *True, llvm::BasicBlock *False,
                                         const int probability = 90);

    llvm::BranchInst *CreateUnlikelyCondBr(llvm::Value *Cond, llvm::BasicBlock *True, llvm::BasicBlock *False,
                                           const int probability = 90) {
    ...
    llvm::Value *CreateCountForwardZeroes(llvm::Value *value, const llvm::Twine Name = "",
                                          const no_conversion<bool> guaranteedNonZero = false);
    llvm::Value *CreateCountReverseZeroes(llvm::Value *value, const llvm::Twine Name = "",
                                          const no_conversion<bool> guaranteedNonZero = false);
    ...
    llvm::Value *CreateZeroHiBitsFrom(llvm::Value *bits, llvm::Value *pos, const llvm::Twine Name = "");
    ...
    virtual llvm::LoadInst *CreateLoad(llvm::Type *type, llvm::Value *Ptr, bool isVolatile,
                                       const llvm::Twine Name = "");
    ...
    llvm::LoadInst *CreateAlignedLoad(llvm::Type *type, llvm::Value *Ptr, const unsigned Align,
                                      const llvm::Twine Name = "");

    llvm::LoadInst *CreateAlignedLoad(llvm::Type *type, llvm::Value *Ptr, const unsigned Align, bool isVolatile,
                                      const llvm::Twine Name = "");

    llvm::StoreInst *CreateAlignedStore(llvm::Value *Val, llvm::Value *Ptr, const unsigned Align,
    ...
    llvm::Value *CreateInsertElement(llvm::Value *Vec, llvm::Value *NewElt, llvm::Value *Idx,
                                     const llvm::Twine Name = "");
    ...
    void setDriver(BaseDriver &ts) { mDriver = &ts; }

    BaseDriver &getDriver() const { return *mDriver; }
...
// LLVM 18 removed all qualified pointer types but if we want to support earlier LLVM versions, we must still still
// allow kernels to construct them.
#if LLVM_VERSION_INTEGER >= LLVM_VERSION_CODE(18, 0, 0)
#define ADD_POINTER_TYPE_ALIAS(Name)                                                                                   \
    llvm::PointerType *get##Name##PtrTy(unsigned = 0) { return llvm::PointerType::getUnqual(getContext()); }
...
#undef ADD_POINTER_TYPE
    ...
    bool hasFeature(const codegen::Feature feature) const LLVM_READNONE { return mFeatureSet.test((size_t)feature); }

  protected:
    llvm::CallInst *__CreatePrintfCall(llvm::Value *const format, std::initializer_list<llvm::Value *> args);

    llvm::CallInst *__CreateDprintfCall(llvm::Value *const fd, llvm::Value *const format,
                                        std::initializer_list<llvm::Value *> args);

    llvm::CallInst *__CreateSprintfCall(llvm::Value *const str, llvm::Value *const format,
                                        std::initializer_list<llvm::Value *> args);
...
#endif
    codegen::FeatureSet mFeatureSet;
};
...
template <typename T, unsigned N> using FixedArray = std::array<T, N>;
